...
b = {k: open(f"{F}/{k}.png.b64").read() for k in
     ["encoders", "layers", "transfer", "acoustics", "replication", "room", "leakage", "matrix3", "pigs", "bats", "construct", "encoders3"]}
...
<figure><img src="data:image/png;base64,{b['encoders3']}" alt="Six transfer cells across three encoders"><figcaption><b>Three encoder families, same six cells. No cell is unanimous.</b> WavLM finds dog&rarr;cat and pig&rarr;cat; ESP&rsquo;s bioacoustic AVES finds pig&rarr;cat and cat&rarr;pig; HuBERT finds dog&rarr;cat only at p=0.06. <b>pig&rarr;cat is significant in two of three.</b> Every encoder finds at least one species that predicts cat contexts above chance &mdash; but <em>which</em> species is encoder-dependent. Placebos stay at chance throughout.</figcaption></figure>
<div class="note bad"><h4>What that means for the transfer claim</h4><p>It is real, small, and not a stable code. The honest sentence: <em>across three frozen encoders, an affect axis learned on one species predicts cat contexts above chance in every encoder, but the source species that transfers is encoder-dependent and no cell is unanimous; roughly half is call duration; read it as &ldquo;encoders share some coarse arousal structure across mammals,&rdquo; not as animals sharing a signal.</em> That is a publishable cautionary result, and it is the opposite of a headline.</p></div>
...
<li><b>Paper B, transfer &mdash; smaller and stranger.</b> Real in every encoder, but encoder-dependent in <em>which</em> cell; pig&rarr;cat 2 of 3, dog&rarr;cat 1 of 3 plus a borderline; placebo-clean, survives duration partialling (borderline), balanced classes, strict scaling, group-level permutation, construct validity; isolation hypothesis rejected. But only <em>into</em> cats, and only one encoder family tested at scale. Honest framing: <em>a cat affect axis is predictable from probes trained on two other species; the reverse is not; the shared component is small, partly duration, and not isolation-specific.</em></li>
</ul></div>
</div></section>
