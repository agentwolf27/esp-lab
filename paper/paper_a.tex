...
baseline, without commenting on the disparity. The same encoders are now being turned on identity as
a task: \citet{chauhan2025} fine-tune a bioacoustic encoder into a 173-way individual classifier for
wild zebra finches, splitting by clip, so recordings made in the same tree on the same day fall on
both sides. That is the conventional design for a closed-set task, but it leaves the day and the
recorder untested; the companion field study from the same group \citep{hagedoorn2025} reports
embedding distances that are lower for recordings sharing an hour even between different individuals.
The confound is visible in that system without yet being carried into the evaluation.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/inflation_law.png}
  \caption{Each point is one (dataset, feature set, layer). \textbf{Left:} the better a feature set
  identifies the animal, the more a random split flatters it. \textbf{Right:} the same points with
  each dataset centred on its own mean---the relationship holds \emph{within} all three corpora
  (cats $r{=}0.71$, dogs $r{=}0.59$, pigs $r{=}0.59$). Across corpus means it does not
  ($r{=}0.221$, $n{=}12$), which is the scope we claim.}
  \label{fig:inflation}
\end{figure}
...
Pushed to its limit the same test discards the audio entirely, and for identity it can be run on
metadata alone. The corpus our bat clips are drawn from \citep{prat2017} publishes a file table
giving every recording a timestamp, a microphone channel and a treatment period; across its 83{,}222
annotated calls with an identified emitter (82 emitters, 345 days, 12 channels), Cram\'er's $V$
between emitter and microphone channel is 0.589, which is also what we measure between pig valence and
recording lab (0.59). Channel and date together, with nothing acoustic, identify the emitter at 0.297
against a 0.012 chance rate. Bats roost in fixed positions, so \emph{which bat} and \emph{which
microphone} are close to the same question. We cannot transfer this onto our own bat numbers: the
benchmark packaging we use renumbers file identifiers and does not join to the annotation table
(emitter agreement 0.1\%), so we do not know which subset we hold. The observation is about the
recording design our identity number inherits, not about that number.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/stress.png}
  \caption{Decision flips when the same clips are perturbed, against probes trained on clean audio.
  \textbf{Right:} the ratio uses a \emph{difficulty-matched} binary identity probe, so the comparison
  is not confounded by task difficulty; the unmatched version exaggerates it to 2.7--6.1$\times$.
  Identity is the fragile signal, and most fragile under bandwidth loss---consistent with individual
  identity living in fine spectral detail and affect in coarse envelope structure.}
  \label{fig:stress}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/coverage_paper.png}
  \caption{Split conformal at $\alpha{=}0.10$, calibrated on other individuals or labs and tested on
  a held-out one---the deployment case. One dot per group, area proportional to clip count. Marginal
  coverage lands on the 0.90 target in all three corpora; the guarantee is marginal, and the animal
  is not. The group-conditional (Mondrian) remedy is unavailable here by construction: a group that
  is held out has no calibration data of its own.}
  \label{fig:coverage}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/recovery.png}
  \caption{Labelled clips from the held-out group, added back into training. The individual gap is
  cheap to close and saturates (cats, $k{\approx}11$); the site gap is not (pigs, 100 clips close
  half of it and the curve is still climbing). Ecologists move recorders between sites far more often
  than they change animals, so the expensive case is the common one.}
  \label{fig:recovery}
\end{figure}
...
including the eGeMAPS comparison, should be read with that caveat. \textbf{Our identity numbers are diagnostics, not task results.} Each is measured under a random
split by design, because what a context probe can borrow is identity as it is available at training
time. Read instead as a claim about how well these encoders identify animals, the same numbers would
be inflated by exactly the effect this paper documents. Where identity is the task, the session, the
recorder or the day has to be held out, and we do not do that here. The cross-corpus correlation is underpowered ($n{=}12$). Probes are linear and
...
fruit bats \citep{prat2017}. \paragraph{LLM use.} A large language model was used throughout this work as a coding and drafting
assistant: implementing the experiments, writing the analysis and plotting scripts, searching and
summarising related work, and drafting and revising this text. It was also used adversarially, to
attack our own claims; several results reported here were withdrawn or rescoped as a result, and one
error that survived into an earlier draft---an artefactual below-chance figure caused by imputing
clips absent from a published feature table---was found that way and is reported in
\S\ref{sec:baseline}. Every number in this paper is produced by committed code and checked against
its stored JSON output; every reference was verified against the publisher or a bibliographic
database before citation. The authors take full responsibility for all claims.
