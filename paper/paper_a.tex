...
erasing a 6-point random-split lead; and the corpus's own published feature set scores below chance
(0.386) once a lab is held out. The failure extends to uncertainty quantification: conformal
...
(3)~An unexpectedly strong simple baseline, and a published feature set that falls below chance under
site shift.
...
\citet{lin2026} document an ``identity trap'' in EEG where subject identity is decodable far above the
task label.
...
\citet{barber2021} prove exact conditional coverage is unattainable in general; \citet{ding2023} give
the class-conditional sample-size bound $(1/\alpha)-1$ that we instantiate empirically in
\S\ref{sec:conformal}. We found no prior application of conformal prediction to bioacoustics or
...
Groups are the individual animal (cats, dogs, bats) or the recording lab (pigs). Probes are
...
\subsection{An unexpectedly strong simple baseline}
...
evaluation---below the 0.500 chance level---indicating the feature-to-valence mapping partly inverts
between labs. Our re-analysis uses a linear probe; the original study used other classifiers and did
not claim cross-lab generalisation. This is a reframing under a different protocol, not a refutation.
...
room characteristics ``should not affect the captured audio signals.''
...
leave-one-session-out. The cross-corpus correlation is underpowered ($n{=}12$). Probes are linear and
...
experiment runs on a CPU laptop; total compute is zero GPU-hours. Datasets are public: CatMeows
(Zenodo 4008297, CC-BY-4.0), dog barks \citep{molnar2008}, Soundwel (Zenodo 8252482, CC-BY-4.0), and
Egyptian fruit bats \citep{prat2017}. \textcolor{red}{[LLM-use disclosure required by the venue: state
